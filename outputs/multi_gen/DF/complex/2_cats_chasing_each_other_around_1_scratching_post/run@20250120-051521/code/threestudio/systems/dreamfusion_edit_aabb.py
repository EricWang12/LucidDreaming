...
from threestudio.utils.misc import process_gpt_file, find_center_of_box, plot_bool_tensor, plot_bool_tensor_new, find_width_of_box
import torch.nn.functional as F
import warnings

@threestudio.register("dreamfusion-system-edit-ab")
class DreamFusion(BaseLift3DSystem):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for obj in self.objects:
            ...
            # if this object is global, scene edit will only add to geometry,
            # if self.cfg.global_guidance_start != -1 and (aabb == self.renderer.bbox.view(-1)).all():
            #     loss_weight = self.cfg.global_weight
            # else:
            #     loss_weight = 1.0
        ...
        if self.cfg.scene_edit or self.cfg.recon_loss_weight > 0:
            ...
            # object_aabbs = torch.stack([ob["aabb"].to(self.device) for ob in self.objects])
            
            # if self.true_global_step > self.cfg.global_guidance_start:
            #     object_aabbs = object_aabbs[:-1]
            # ignore the warning of the empty ray in genenration
