...
from threestudio.utils.misc import  find_center_of_box,find_width_of_box, process_gpt_file
...
class ImplicitVolume(BaseImplicitGeometry):
    ...
    class Config(BaseImplicitGeometry.Config):
        ...
        init_side_ratio: float = 0.625 # the higher the more centric the blob is
    ...
    def configure(self) -> None:
        ...
        if self.cfg.gpt_file != "":
            objects = process_gpt_file(self.cfg.gpt_file)
            self.aabbs = [ob["aabb"] for ob in objects]
        else:
            self.aabbs = None
        self.unbounded = self.cfg.unbounded
    ...
    def get_activated_density(
        ...
        if self.cfg.density_bias == "blob_dreamfusion":
        ...
        elif self.cfg.density_bias == "blob_edit":
            # pre-activation density bias
            if self.aabbs is None:
                raise ValueError(f"edit requires aabb")
            if self.aabbs[0].device != self.device:
                self.aabbs = [a.to(self.device) for a in self.aabbs]

            if self.cfg.unbounded:
                density_bias = torch.full(density.shape, -1., device=self.device)
            else:
                density_bias = torch.full(density.shape, float('-inf'), device=self.device)
                # density_bias = torch.full(density.shape, -1., device=self.device)
        
            for aabb in self.aabbs:
                center = find_center_of_box(*aabb, self.device)
                side = find_width_of_box(*aabb, self.device)
                if not (aabb == self.bbox.view(-1)).all():
                    side_normal_to_bb = (side * self.cfg.init_side_ratio)
                else:
                    side_normal_to_bb = side/find_width_of_box(*self.bbox.view(-1), self.device)
                points_centered = (points - center)/side_normal_to_bb
                density_intermiate = (
                    self.cfg.density_blob_scale
                    * (
                        1
                        - torch.sqrt((points_centered**2).sum(dim=-1)) / self.cfg.density_blob_std
                    )[..., None]
                )
                density_bias = torch.max(density_bias, density_intermiate)
            
        elif self.cfg.density_bias == "blob_box":
            # pre-activation density bias
            if self.aabbs is None:
                raise ValueError(f"edit requires aabb")
            if self.aabbs[0].device != self.device:
            ...
            for aabb in self.aabbs:
                ...
                inside = (points[:, 0] >= aabb[0]) & (points[:, 0] <= aabb[3]) & \
                    (points[:, 1] >= aabb[1]) & (points[:, 1] <= aabb[4]) & \
                    (points[:, 2] >= aabb[2]) & (points[:, 2] <= aabb[5])
                    
                density_bias[inside] = self.cfg.density_blob_scale
            # breakpoint()
                # density_bias = torch.max(density_bias, density_intermiate)
