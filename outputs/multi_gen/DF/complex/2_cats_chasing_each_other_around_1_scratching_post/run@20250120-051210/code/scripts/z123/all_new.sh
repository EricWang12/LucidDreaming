...
# gpu=${1:-$DEFAULT_GPU}
IFS=' ' read -r -a gpu_indices <<< "$1"
dir=${2:-"objects/z123"}
cfg=${3:-$DEFAULT_CFG}
# gpu_indices=${3:-(0)}
script=${4:-"run_multi.sh"}
...
for file in "$dir"/*; do
    ...
    # if [[ -d "outputs/multi_gen/$cfg/$cls/$basename_no_ext" ]]; then  # Check if a directory with the basename exists
    #      echo "Skipping outputs/multi_gen/$cfg/$cls/$basename_no_ext"
    ...
    if [[ ${#files[@]} -eq ${#gpu_indices[@]} ]]; then
        for i in "${!files[@]}"; do
            ...
            cmd+="bash scripts/z123/${script} ${gpu_indices[$i]} ${files[$i]} ${cfg} & "
        done
...
if [[ ${#files[@]} -gt 0 ]]; then
    ...
    cmd=""
    for i in "${!files[@]}"; do
        cmd+="bash scripts/multi_gen/${script} ${gpu_indices[$i]} ${files[$i]} ${cfg} & "
    done
