...
from threestudio.utils.misc import find_center_of_box
...
class NeRFVolumeRenderer(VolumeRenderer):
    ...
    def configure(
        ...
        if self.cfg.estimator == "occgrid":
            # tmp_box= copy.deepcopy(self.bbox)
            # tmp_box[1]=torch.tensor([0,0,2])
            # tmp_box[0]=torch.tensor([-2,-2,0])
            # self.estimator = nerfacc.OccGridEstimator(
            #     roi_aabb=tmp_box.view(-1), resolution=32, levels=1
            # )
            self.levels = self.cfg.levels if self.cfg.unbounded else 1
            self.estimator = nerfacc.OccGridEstimator(
                roi_aabb=self.bbox.view(-1), resolution=self.cfg.grid_resolution, levels=self.levels
            )
        ...
        if self.cfg.unbounded:
            # self.cone_angle = 10**(math.log10(self.cfg.far_plane) / self.cfg.num_samples_per_ray) - 1. # approximate
            self.cone_angle = 0.004 # approximate

            self.render_step_size = 0.001 
            # self.cfg.alpha_thre = 0.0

        else:
            self.cone_angle = 0.0


    def set_aabb(self, aabb: Tensor):
        if aabb is None:
            self.aabb = None
        elif self.cfg.estimator == "occgrid":
            aabb = aabb.reshape(-1,6)
            self.aabb = aabb
    ...
    def forward(
        ...
        **kwargs
        ...
        if hasattr(self, "aabb") and self.aabb is not None:
            # temp = torch.tensor([0,3,0]).to(self.device)
            # temp_flatten = temp.repeat(1, 64, 64, 1).view(-1,3)
            # temp = torch.cat([temp,temp])
            t_min, t_max, hit =  nerfacc.ray_aabb_intersect(rays_o=rays_o_flatten, rays_d=rays_d_flatten, aabbs=self.aabb, near_plane=self.cfg.near_plane, far_plane=self.cfg.far_plane)
            t_min, t_max, hit = t_min.view(-1), t_max.view(-1), hit.view(-1)
        else:
            t_min, t_max, hit = None, None, None
        if self.cfg.estimator == "occgrid":
            if not self.cfg.grid_prune:
                with torch.no_grad():
                ...
                def call_sampling_with_minmax(t_min, t_max):
                    with torch.no_grad():
                        ray_indices, t_starts_, t_ends_ = self.estimator.sampling(
                            rays_o_flatten,
                            rays_d_flatten,
                    ...
                    return ray_indices, t_starts_, t_ends_
                
                if 'void_aabb' in kwargs:
                    t_min, t_max, hit =  nerfacc.ray_aabb_intersect(rays_o=rays_o_flatten, rays_d=rays_d_flatten, aabbs=self.aabb, near_plane=self.cfg.near_plane, far_plane=self.cfg.far_plane)

                    ray_indices, t_starts_, t_ends_ =  call_sampling_with_minmax(t_min=None, t_max=None)
                    pos = (t_starts_ + t_ends_) / 2.0

                    mask = (pos[:, None] < t_min[ray_indices]) | (pos[:, None] > t_max[ray_indices])
                    mask = mask.all(dim=1)
                    
                    ray_indices = ray_indices[mask]
                    t_starts_ = t_starts_[mask]
                    t_ends_ = t_ends_[mask]
                    
                else:
                    ray_indices, t_starts_, t_ends_ = call_sampling_with_minmax(t_min=t_min, t_max=t_max)
                    # ray_indices, t_starts_, t_ends_ =  call_sampling_with_minmax(t_min=None, t_max=None)
                    #              # Compute the mid-points
                    # pos = (t_starts_ + t_ends_) / 2.0
                    # mask = (pos < t_min[ray_indices]) | (pos > t_max[ray_indices])
